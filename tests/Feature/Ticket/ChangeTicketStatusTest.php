...
class ChangeTicketStatusTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_user_can_change_ticket_status(): void
    {
        ...
        $response = $this->actingAs($data['admin'], 'sanctum')->postJson("/api/tickets/{$ticket->id}/status", [
            'status_id' => $data['statusAndamento']->id,
        ]);

        $response
            ->assertOk()
            ->assertJsonPath('status.slug', 'em_andamento');
    }
}